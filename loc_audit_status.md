...
**2026-06-01 (later) — whole-language native-review Workflow pass (all 20 targets):** a second,
*coherence*-focused pass layered on the batched re-sweep — one full-context native reviewer + one
independent skeptical validator **per language**, each holding the ENTIRE language (the cross-key
angle the 200-key batches are structurally blind to). **90 fixes / 76 keys across 18 langs applied
(`dirty`+`unverified`), `make lint` 0/0; `da` + `ru` came back clean (0 fixes).** **Push PENDING
operator** (`make push` — 18 langs, 76 keys; agent has no token, [CR-ACCESS]). Until pushed, the
per-row `pushed ✓` above is **stale for these 76 keys**. Detail + per-lang counts + the
rate-limit-safe vehicle: § Execution archive top entry.
...
- **2026-06-01 (later) — whole-language native-review Workflow pass, all 20 targets; 90 corpus edits across 76 keys (18 langs), `dirty`+`unverified`; PUSH PENDING operator.** A *coherence* pass layered on the same-day batched re-sweep: the operator explicitly wanted a whole-language **native review** (not a structural audit) — **one full-context native reviewer + one independent skeptical validator per language**, each holding ALL 1261 keys at once (1M context) to catch the cross-key consistency the 200-key batches are structurally blind to. Vehicle: one dynamic Workflow, **40 agents / ~10.1M tokens / ~41 min**. Operator scope = error **+** style (warn), validator-gated.
  - **Yield — 90 across 76 keys (ja 21, tr 16, hi 6, vi 6, de 5, ko 5, nl 4, pl 4, es 3, id 3, ms 3, nb 3, pt_BR 3, it 2, sv 2, zh_CN 2, ar 1, fr 1; `da` 0, `ru` 0).** Dominant class = **cross-key glossary / brand-freeze drift only a whole-language read catches:** ja ドリンク→飲み物 (19-key sweep; 飲み物 already dominates 50+ keys), tr brand calque `Benim suyum`→`Suyum` (12 keys; matches CFBundle*), sv/nb frozen tier `My Water Premium` restored from `Mitt Vatten`/`Mitt Vann Premium` (32:2 / sibling-confirmed), pt_BR `objetivo`→`meta` (glossary CANONICAL, gender-sibling-confirmed), zh_CN/vi/ko volume + spacing term unification. Plus per-key: es `notifErrorM/F` semantic-drift (asserted a user action absent from en/ru, dropped the "in Settings" recovery), fr + zh_CN Siri prompt de-UI-fied (parenthetical text-field hint → natural spoken question), ar `شروط الخدمة`→use-form `شروط الاستخدام` (glossary), id `Anda`→`-mu` T-form on non-legal surfaces, pl brand-quote/casing.
  - **Independent-validator value:** the `ko` validator ADDED a reviewer MISS (`text3_7F`, genderless-divergence unified onto the en-accurate `*M` sibling → apply_count 4→5); `tr` and `ms` 1 modify each. Rejections were few (reviewers ran conservative — the corpus was audited the same day), but `da`/`ru` correctly yielded zero (no churn). Every suggestion cleared the new-translation bar (no calque / clinical term / em-dash / V-form leak introduced).
  - **Gate (standing discipline, all passed):** structural pre-check (0 unmatched / 0 em-dash U+2014 / 0 plural-type-mismatch over the 90), **test-applied to a `--corpus` copy** (`replaced==apply_count`, changed-set==apply-set over 76 keys, **0 cross-language collateral**, copy `make lint` 0/0), serial single-writer `make apply`, **live==copy byte-identical**, live `make lint` 0/0. Target-only pass — no `en` source edits applied (reviewer en-observations are operator notes, not applied; [CR-CORPUS-SOURCE-CHANGE]).
  - **⚠️ Rate-limit incident + safe-launch recipe (durable vehicle lesson, distinct from the § Workflow 0 agent-COUNT ceiling).** The FIRST attempt threw all 20 langs into a `pipeline()` at the workflow's **default 16-wide concurrency**, each agent **paging its ~16,010-line extract in ~1000-line chunks** — every chunk re-sends the growing context (triangular), so ~16 agents × ~1.7M tok ≈ **~28M tokens in ~28s → server-side throttle** ("temporarily limiting requests"); all 20 agents died mid-read, 0 applied. Root cause = **throughput burst**, a different axis from agent count. Fix (relaunch ran clean): (1) **single big `Read` (`limit 16100`)** per agent — never chunked, kills the triangular re-send; (2) **worker-pool concurrency cap = 3** languages for that relaunch, not 16 (the durable § Workflow 0 cap was since raised to **~6** for heavy passes — with the single big `Read` killing the triangular re-send, ~6 concurrent one-shot reads stay well under the burst; the 3 here was pre-fix caution); (3) **drop the StructuredOutput schema** — agents write the `/tmp/loc_<lang>.json` artifact (the real deliverable) + return plain text, so a throttled agent fails cleanly instead of looping on "didn't call StructuredOutput", and file-presence is the success signal (delete stale prior-run `/tmp` files first). Canonized as a § Workflow 0 bullet in `loc_audit_prompt.md`.
