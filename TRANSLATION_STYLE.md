...
- audit sub-agent — `loc_audit_prompt.md` **операционализирует** эти правила (flag/skip/severity) дословно для Opus 4.8 sub-agent'а; per-language калибровки — `loc_audit_lang_calibration/<lang>.md`;
...
- **Длинное тире (em-dash `—`, U+2014) в user-facing строках — НЕ использовать.** Его пишут почти только AI вместо обычного тире — многим это не нравится. Три разных символа, AI их путает — не считать за один: длинное тире `—` U+2014 (**запрещено** в user-facing значениях) / en-dash `–` U+2013 (проектом не регулируется — отдельно не вводить, но и не «чинить» drive-by; это не em-dash) / обычное тире (hyphen-minus `-` U+002D, **разрешено**, в т.ч. как разделитель между слов `« - »`). **Дефолт — перестроить пунктуацию по смыслу, а не механически подставить дефис.** Длинное тире в user-facing строке — чаще всего симптом рыхлого предложения: там, где оно соединяет ярлык со значением или две части, чище читаются двоеточие, точка или запятая. Google в своём style guide для документации разработчиков для пары «пункт — описание» рекомендует **двоеточие или точку вместо тире** (их пример: `Example: This is an example.`, не `Example - This is an example.`) и не использовать тире как разделитель в описаниях ([developers.google.com/style/dashes](https://developers.google.com/style/dashes)). Поэтому ориентир замены = `en`-значение того же ключа: **запятая** для параллельных императивов (`Пей воду, будь в тонусе!`); **точка** для двух самостоятельных предложений / Siri-voice (`…daily goal. %1$@ of %2$@. Great job!`); **двоеточие** для label↔value (`%1$@: %2$d%% of total`); либо реструктуризация, убирающая связку вовсе. Обычное тире `-` — лишь **fallback**, когда перестройка не улучшает casual-строку. Для `ar` / CJK — script-correct знак (`ar` `،` U+060C; CJK `，` / `、`). Применять во всех 21 языке. Область правила — **только user-facing значения**; em-dash как канонический знак русской прозы в doc-прозе (`mywater_ios docs/ai/DOCS_MAINTENANCE.md`) этим правилом НЕ затрагивается. Детерминированно проверяется `loc_qa.py` (em-dash check, ERROR; user-facing значения, не `context`).
...
/*
  ...
  Type: <structural role — primary role first; «role A / role B» for a genuinely multi-surface string; optional `(subtype)` qualifier — see the role list below>
...
- **Type** — структурная роль элемента (что это за UI-кусок: кнопка / заголовок / абзац / уведомление…). Снимает неоднозначность для переводчика (`Save` — глагол-кнопка или существительное-label?). **Это информативная роль, а не закрытый словарь.** Список ниже — рекомендованный набор имён: держись его, где подходит (единообразие помогает аудиту), но (а) если строка реально живёт на нескольких поверхностях — указывай **мульти-роль через `/`** (`settings row title / screen title`), не выбирай одну силой; (б) если подходящего имени нет — назови роль естественно, не втискивай в `paragraph` / `label`. **Первой ставь первичную роль:** audit группирует ключи для single-surface-консистентности (`loc_audit_prompt.md` rule #9 / skip rule #4 «inconsistent with neighbors in the same comment-Type bucket») по **нормализованному первому токену** Type — он отбрасывает `(subtype)`-уточнитель и хвостовую em-dash-клаузу и берёт первый `/`-сегмент, поэтому `settings row title / screen title` и `settings row title` попадают в один бакет. Допустим необязательный жанрово-/тональный уточнитель в скобках (`paragraph (marketing claim)`, `section header (eyebrow)`, `status message (multi-line)`) — это под-тип. Значение — **без trailing-точки** (`motivational text`, не `motivational text.`). Соответствие списку **не enforced** (`loc_qa.py` его не проверяет — намеренно: жёсткая валидация по словарю кромсала информативные роли и расщепляла бакеты на мульти-ролях, давая шум без пользы для перевода). Рекомендованный набор ролей (ориентир, пополняй по мере надобности):
...
- `loc_audit_prompt.md` — audit sub-agent prompt + workflow: **операционализирует** этот канон (flag/skip/severity/output) дословно для Opus 4.8 sub-agent'а.
- `TRANSLATION_EXAMPLES.md` — worked Bad→Good примеры (translation + translator-context), вынесены из канона; append-only reference.
- `loc_audit_lang_calibration/<lang>.md` — per-language калибровки (ar / hi / vi / id / ms / pl): script / plural CLDR / per-language calque / skip rules.
