...
**Default register reminder (ru / de / fr / es / it / pl / tr / nl / pt-BR / zh-Hans / hi / id / ms / vi и аналогичные T-V / honorific языки):** дружелюбная неформальная форма — **friendly T-form («ты»)** — это **универсальный регистр для всех поверхностей без исключения**. Прежний `ты`/`вы` split (casual surfaces на «ты», а формальные — App Store / paywall / educational / legal / permission prompts / errors / Siri — на «вы») **устарел** (operator decision 2026-05-30) и больше не применяется: формального V-form carve-out'а больше нет. Старые строки могут нести legacy «вы» — они грандфазерятся (не sweep'ить, vendor-консоль Lokalise — owner переводов); новые строки на любой поверхности — только «ты». Полное правило, per-language карта и pre-commit detection signal — `§ Brand voice § Pronouns`. AI default бесконтрольно вытягивает V-form из training data; проверять каждую новую translation на imperative suffix `-ите` / `-айте` / `-йте` / `-ьте` и pronoun `вы` / `ваш` ещё до записи в файл — детектор в `§ Brand voice § Pronouns § Pre-commit detection signal (ru)`.
...
❌ "Пожалуйста, дайте нам разрешение на доступ к вашей камере, чтобы сделать фотографию на аватар"  — V-form: устаревший подход, permission prompt больше не formal surface (split deprecated 2026-05-30)
✓ "Дай нам доступ к камере, чтобы сделать фото на аватар"  — T-form: дружелюбная форма теперь универсальна для всех поверхностей

EN: "No Internet connection. Please check your connection and try again later."  (error with recovery instructions)
❌ "Нет соединения с сервером. Пожалуйста, проверьте ваше интернет-соединение и попробуйте снова позже."  — V-form: устаревший подход (split deprecated)
✓ "Нет соединения с сервером. Проверь интернет-соединение и попробуй снова позже."  — T-form: дружелюбная форма универсальна, включая ошибки
...
- **User → "you" / "your"** — direct address на «ты-эквиваленте» по нормам target language. Примеры: «Your body will thank you», «You did it!», «Take care of yourself today».
  ...
  - **⚠️ `ты`/`вы` split — устаревший подход (2026-05-30, operator decision).** Прежняя модель — surface-based split, где casual-поверхности на «ты», а формальные (App Store, paywall hero / CTA, onboarding-educational, legal / Terms / Privacy, permission prompts, errors, Siri AppIntent description) на «вы» — **устарела и больше не применяется**. Теперь дружелюбная неформальная форма — **«ты»** (T-form-эквивалент в каждом T-V / honorific языке — см. per-language карту ниже) — это **универсальный регистр для всех поверхностей без исключения**. Формального V-form carve-out'а больше нет: ни одна поверхность не оправдывает «вы».
    - **Старые строки грандфазерятся.** Строки, написанные при старом split, могут нести legacy «вы» — их **не** переписывать drive-by, mass-sweep **не** запускать (vendor-консоль Lokalise — owner переводов; `mywater_ios docs/ai/COMMON_MISTAKES.md § [CM-LOCALE-MASS-FANOUT]` / `§ [CM-MECHANICAL-GREP-SWEEP]`). «Старые фразы могут быть устаревшими» — это ожидаемое следствие смены подхода, не дефект-к-немедленной-правке.
    - **Новые строки — только «ты».** Любая новая строка, rewrite при смене `en`-источника, или переименованный ключ — сразу в дружелюбной форме, на всех поверхностях, без grace period.

  - **Default — friendly T-form («ты»), универсально.** Это основная формулировка для **всего** user-facing текста в языках с T-V / honorific split. AI training data перекошен в сторону formal V-form («Добавьте текст», «Введите значение» — register IT-документации и customer service), поэтому первый instinctive candidate в ru / de / fr / es и т.д. почти всегда V-form. Это нужно сознательно отвергать на write-time; T-form — не stylistic option, а brand-voice default.

  - **T-form обязателен везде; casual-поверхности ниже — там, где V-form leak исторически самый частый (проверять в первую очередь):**
  ...
  - **❌ Formal V-form («вы») surfaces — DEPRECATED (split устарел; теперь и они на «ты»).** Список ниже — *исторический*: это поверхности, которые при старом подходе несли «вы». Теперь они все на «ты», как и весь остальной UI. Список сохранён только чтобы (а) знать, где в корпусе сидит legacy «вы» под grandfather, и (б) не принять старый «вы» за «намеренный формальный регистр» — он больше **не** намеренный. Новые строки на этих поверхностях — «ты»:
    ...
    - Paywall CTA buttons / упоминания offer условий («Попробуй 7 дней бесплатно», «Активируй подписку») — раньше это был intentional single-register formal V-form в единстве с paywall hero; теперь paywall целиком (и hero, и CTA) — «ты», как и всё остальное. Legacy «вы» здесь грандфазерится (no sweep); новые CTA — «ты». Прежний «outlier» `reachTheGoal` = «Достигни цели» (casual-T) теперь не outlier, а норма.
    ...
    На этих surfaces V-form больше **не** intentional. Но legacy «вы» под grandfather не флагать как срочный дефект (no mass sweep) — просто новые строки писать на «ты». Операционализация для audit — `loc_audit_prompt.md` skip rule #1 (deprecated) + rule #8.

  - **Key-level Register signal — write-time anchor против V-form leak.** Канонический per-key анкор — поле `Register:` в translator-context (`§ Translator context § Опциональные поля`). Рабочие значения теперь: `Register: casual T-form` (дефолт практически везде) и `Register: neutral` (settings-регистр). Значение **`Register: formal V-form` устарело** вместе со split'ом: на новых ключах его не ставить; на legacy-ключах оно читается как «здесь сидит grandfathered «вы», не флагать как срочный дефект», а **не** как «намеренный формальный регистр». Поле фиксирует решение на write-time, чтобы AI-переводчик (training-prior которого тянет к formal V-form) не делал V-form leak. Cross-platform — применимо ко всем 16 T-V / honorific языкам корпуса; на Scandinavian / `en` работает как tone-shift hint (lexical formality), не pronoun-swap. Audit-sub-agent читает `Register:` как authoritative override; отсутствие `Register:` означает дефолт — а дефолт теперь T-form на любой поверхности (бывшие formal — под grandfather).

  - **Pre-commit detection signal (ru) — обязательная проверка перед записью любого нового перевода (на любой поверхности — V-form теперь off-brand везде):**
    ...
    - Legacy «вы» на любой поверхности (включая бывшие formal) — под grandfather: drive-by не править, mass-sweep не запускать; но новую строку всегда писать на «ты».

  - **Legacy V-form на любой поверхности — grandfather, не drive-by.** Существующие ключи с «вы» — и casual (`tempAddOneTap`, `forgotWriteText`, `manyDrinksText`, `noAdText`, `tempFirstSteps`, `widgetText`), и бывшие formal (App Store, paywall, permission prompts, errors, legal, Siri) — наследие предыдущего split-подхода; не переписывать drive-by при touched-strings задачах. Mass sweep по `rg`-сигналу `-айте` / `-ите` в casual surface = `mywater_ios docs/ai/COMMON_MISTAKES.md § [CM-MECHANICAL-GREP-SWEEP]` поверх `§ [CM-LOCALE-MASS-FANOUT]`: vendor console Lokalise — canonical owner переводов, платформенный PR не инициирует mass retranslation.
    ...
    - Новые ключи на любой поверхности — сразу T-form, никакого grace period.
  ...
  - **Pronoun economy — prefer pronoun-free / impersonal phrasing where natural.** Русский часто не требует местоимения вовсе; passive / impersonal / stative конструкции звучат native и не вытягивают `ты` в каждой строке. Это **не отмена** T-form default — это второй слой на нём: когда прямое обращение действительно нужно — `ты` / `твой`; когда фраза работает без местоимения — лучше без местоимения. Wellness habit-tracker по своей природе про личную привычку и тело; постоянное `ты выпил` / `ты достиг` / `у тебя` начинает звучать как сопровождающий комментатор, а не как partner-app.
    ...
    - **Default ranking** для нового перевода (любая поверхность):
      ...
      3. **V-form `вы` / `ваш`** — **устарело** (split deprecated): в новых строках не использовать ни на какой поверхности. Legacy «вы» — под grandfather.
  ...
  - **Применять как pronoun-swap** (T-form / V-form грамматически различны) — но V-form теперь **deprecated на всех поверхностях**: правая колонка ниже остаётся только как reference для распознавания legacy-V-form leak, не как валидный регистр для новых строк. T-form (левая) — единственный живой регистр: `ru` (ты / ~~вы~~), `de` (du / ~~Sie~~), `fr` (tu / ~~vous~~), `es` (tú / ~~usted~~), `it` (tu / ~~Lei~~), `pl` (ty / ~~Pan / Pani~~), `tr` (sen / ~~siz~~), `nl` (jij / ~~u~~), `pt-BR` (você / ~~o senhor~~), `zh-Hans` (你 / ~~您~~), `hi` (tum / ~~aap~~), `id` (kamu / ~~anda~~), `ms` (awak / ~~anda~~), `vi` (kinship-based: `bạn` по умолчанию — не грамматический T-V, см. calibration).
  - **Применять как tone-shift, не pronoun-swap** (T-V distinction отсутствует или V почти не используется): `en`, `ar`, `da`, `nb`, `sv` — прежний formal регистр (deprecated) отличался lexical register / sentence structure, не местоимениями; теперь везде дружелюбный informal регистр (legacy formal — grandfather).
  - **Применять через honorific levels** (отдельная система формальности, не binary T-V): `ja` (敬語 keigo / です/ます ↔ casual), `ko` (해요체 / 합니다체 ↔ 해체) — прежде formal surfaces брали higher honorific level (deprecated); теперь везде casual / informal level (legacy formal — grandfather).
...
/*
  ...
  Register: <casual T-form | neutral — `formal V-form` устарел (split deprecated 2026-05-30); см. § Brand voice § Pronouns>
...
- **Register** — `casual T-form` | `neutral`. (`formal V-form` — **устаревшее** значение: `ты`/`вы` split deprecated, `§ Brand voice § Pronouns`; на новых ключах не ставить, на legacy читается как «grandfathered «вы», не срочный дефект».) Служит явным сигналом write-time AI-переводчику, чьи training-priors перекошены в сторону formal V-form. Когда указывать:
  ...
  - **Бывшие формальные surfaces** (permission prompts, App Store, paywall hero/CTA, error-with-recovery, Siri educational placeholder, legal notice) — теперь тоже `casual T-form` (formal carve-out устарел, split deprecated). На новых таких ключах ставить `Register: casual T-form`, чтобы зафиксировать смену подхода; legacy «вы» там — grandfather.
  ...
  - Семантика значений: `casual T-form` = ты / du / tu / tú / 你 / tum (не आप) / kamu / awak / 해요체-default / casual colloquial-MSA / bạn. `formal V-form` (вы / Sie / vous / usted / 您 / आप / Anda / 합쇼체 / 敬語 / fuṣḥā-formal) — **устаревшее** значение (см. выше), оставлено только как reference для распознавания legacy. Для ja / ko / ar / vi проговаривать кратко: `Register: casual (해요체)` / `casual (bạn)` / casual colloquial-MSA.
  - **Когда Register conflicts с Tone:** Tone — affective слой (encouraging / playful / celebratory / promotional / urgent). Никогда не использовать Tone для T-/V- выбора (`Tone: friendly` ≠ T-form сигнал, его надо записать в Register). Tone применяется поверх Register: `Register: casual T-form` + `Tone: celebratory` для awards-congratulation; `Register: casual T-form` + `Tone: reasoned` для permission prompts (бывший formal surface — теперь тоже T-form). Tone редко указывают на settings / labels / generic — default tone берётся из Surface/Type через `§ Brand voice § По типу поверхности`.
...
/*
  ...
  Register: casual T-form — onboarding educational paragraph; прежний formal V-form carve-out устарел (split deprecated 2026-05-30), теперь дружелюбная форма и здесь. See § Brand voice § Pronouns.
*/
...
/*
  ...
  Register: casual T-form — paywall marketing hero; прежний formal V-form carve-out устарел (split deprecated), paywall целиком теперь на «ты». See § Brand voice § Pronouns.
*/
