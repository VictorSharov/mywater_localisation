...
**Граница со платформенной механикой.** Этот док — про *стиль и смысл* перевода. Платформенная *механика* кодирования строк живёт у каждой платформы и сюда не дублируется: iOS `.strings` / `.stringsdict` / `R.swift` / target membership / `|R|`-маркер в `en.lproj` — `mywater_ios docs/LOCALIZATION.md`; server locale-pack формат `{translation, notes}` — `mywater_server resources/locale/CLAUDE.md`; сериализация корпуса + маркер `unverified` — `CLAUDE.md § [CR-CORPUS-OWNER]` / `§ [CR-CORPUS-UNVERIFIED]`.
...
**Маркер «не верифицировано» (`|R|` / `unverified`) при translation-задаче.** Даже когда оператор явно поставил задачу перевести (этот раздел применим) и переводы делает агент, маркер незавершённости **не снимать**: перевод выполнен AI и ещё не верифицирован человеком. Снятие — отдельное действие **только** по явному запросу оператора либо когда human-verified перевод приходит round-trip через Lokalise. Завершённое состояние translation-задачи = переводы записаны + маркер на месте. Canonical owner маркера (общий для корпуса / iOS / сервера) — `CLAUDE.md § [CR-CORPUS-UNVERIFIED]` (корпус `unverified`; iOS `|R|` в `en.lproj`; server `|R|` в начале `notes`).

**Default register reminder (ru / de / fr / es / it / pl / tr / nl / pt-BR / zh-Hans / hi / id / ms / vi и аналогичные T-V split языки):** основная формулировка — **friendly T-form («ты»)** для casual surfaces (notifications, motivational, tips, awards, empty states, beverage / achievement names, widget hints, in-app feature cards). Formal V-form («вы») — exception для App Store / paywall hero / educational / legal / permission prompts / errors / Siri AppIntent description (полный список и pre-commit detection signal — `§ Brand voice § Pronouns`). AI default бесконтрольно вытягивает V-form из training data; проверять каждую новую translation на imperative suffix `-ите` / `-айте` / `-йте` / `-ьте` и pronoun `вы` / `ваш` ещё до записи в файл — `mywater_ios docs/ai/COMMON_MISTAKES.md § [CM-LOCALE-V-FORM-LEAK]`.
...
- **User → "you" / "your"** — direct address на «ты-эквиваленте» по нормам target language. Примеры: «Your body will thank you», «You did it!», «Take care of yourself today».
  ...
  - **Применять как pronoun-swap** (T-form / V-form грамматически различны): `ru` (ты / вы), `de` (du / Sie), `fr` (tu / vous), `es` (tú / usted), `it` (tu / Lei), `pl` (ty / Pan / Pani), `tr` (sen / siz), `nl` (jij / u), `pt-BR` (você / o senhor), `zh-Hans` (你 / 您), `hi` (tum / aap), `id` (kamu / anda), `ms` (kamu / anda), `vi` (адресация по возрасту / статусу).
...
- Anti-pattern catalog (iOS, кросс-платформенный по смыслу): `mywater_ios docs/ai/COMMON_MISTAKES.md` — `[CM-LOCALE-V-FORM-LEAK]` / `[CM-EN-SOURCE-RU-CALQUE]` / `[CM-EM-DASH-OVERUSE]` / `[CM-LOCALE-MASS-FANOUT]` / `[CM-LOCALE-R-MARKER-STRIP]`.
