...
**आप — universal-तुम default, narrow legal carve-out (2026-05-30 hybrid; `TRANSLATION_STYLE.md § Brand voice § Pronouns`).** Friendly तुम is the default on permission prompts, paywall hero/CTA, error recovery and first-time auth CTAs — reserved tone, not आप. The **one** carve-out is genuinely **legally-binding text**: Terms of Use, Privacy Policy, subscription **legal** terms, and legally-weighted consent (a binding consent contract — distinct from a routine HealthKit / sensitive-data *permission prompt*, which stays तुम). Legal text is drafted impersonally (Sanskritic / nominal register, उपयोगकर्ता…); आप appears only as the direct-address fallback (`Register: formal`). Do **not** *defend* an आप target on a former-formal **non-legal** surface as "intentional formal" — but legacy आप there is **grandfathered** (no mass sweep; flag only clearly new / source-changed, severity `warn`). New strings: तुम on every non-legal surface, `formal` (आप / impersonal) on legal-binding text. (The तू-intimate leak above is unchanged — still wrong everywhere, including legal.)
