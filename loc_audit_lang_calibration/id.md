...
1. **`Anda` overuse** — most common LLM mistake: defaulting to formal `Anda` everywhere because EN "you" lacks register. In a friendly hydration app, default expectation is `kamu` / `-mu`. Flag `Anda` outside legal / paywall-legal / serious error / Terms surfaces.
