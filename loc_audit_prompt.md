...
8. **V-form / formal-register leak — friendly T-form is the UNIVERSAL register (all surfaces).** Per `TRANSLATION_STYLE.md § Brand voice § Pronouns` (operator decision 2026-05-30) the `ты`/`вы` split is **deprecated** — friendly T-form is the brand-voice default for **every** surface; the formal-surface carve-out (App Store / paywall / educational / legal / permission / error / Siri) is retired and no longer defends V-form as "intentional." **Flag** casual-surface V-form / honorific (notification, motivational, tip, award, empty state, beverage / achievement name, widget hint, in-app feature card / upsell) as `brand-voice` — this is the confirmed-defect class (OPERATOR POLICY below). **Former-formal surfaces: grandfather, do NOT mass-flag.** Their legacy «вы» is off-brand under the new policy, but the operator explicitly declined a re-translation sweep ("старые фразы могут быть устаревшими"; vendor console owns translations — `CM-LOCALE-MASS-FANOUT`); surface such a key as `brand-voice` severity `warn` (advisory) ONLY when it is clearly a new / source-changed string — never as a project-wide consolidate-sweep. New strings on ANY surface: T-form is mandatory.

   **Key-level `Register:` field — authoritative override when present.** If the entry's comment block carries an explicit `Register:` line (`Register: casual T-form` / `Register: formal V-form` / `Register: neutral`; canonical owner — `TRANSLATION_STYLE.md § Translator context § Опциональные поля`), treat it as the authoritative register decision for that key:
   - `Register: formal V-form` → **deprecated value** (split retired 2026-05-30): on a legacy key it marks grandfathered «вы» — do NOT flag it as a must-fix leak (no sweep), but it no longer endorses V-form as "intentional formal"; new keys must not carry it.
   - `Register: casual T-form` → rule #8 binding, flag V-form target as a leak even if Surface looks ambiguous; the operator-confirmed register is T-form.
   - `Register: neutral` → suppress rule #8 register-related findings (settings-register surfaces and similar own neutral register); per-variant typo / calque / semantic checks remain active.
   - **Absence of `Register:` does NOT auto-trigger a must-fix.** The default register is now T-form on **every** surface (split deprecated), but former-formal surfaces are grandfather — skip the sweep there. On the casual-Type leaf set ({tip, tip headline, motivational text, notification body / title, achievement description}) the field sits at ~100% post the 2026-05-28 backfill — there a V-form target is a flaggable leak (confirmed-defect class).

   **OPERATOR POLICY (2026-05-16, Phase 3 sweep — binding):** casual-surface V-form / honorific is a **CONFIRMED project-wide defect to fix across all 19 languages**. It is NOT an intentional single-register locale choice (the "may be deliberate like the historical paywall-CTA single-register" consideration is resolved — it is not). Therefore: (a) enumerate **every** casual-surface V-form key as its own individual finding row — do NOT consolidate V-form occurrences into one Summary note, and do NOT defer the question as "an operator-level call"; (b) where `ru` (proven anchor) uses casual T-form on the same key, the T-form rewrite is the expected fix; (c) the validator stage must **accept** these findings (not reject them as stylistic / intentional-register). (**Update 2026-05-30:** skip rule #1's formal-surface carve-out is now **deprecated** — those surfaces are no longer V-form by design either. But this 2026-05-16 sweep was scoped to **casual** surfaces; the former-formal surfaces are **grandfathered, not added to the sweep** — see rule #8 and the 2026-05-30 changelog entry.)

   **CARRIED RU-PARITY ITEM — RESOLVED (2026-05-16, pre-G6, operator-gated).** The carried open question (G3-Delta5 / G4 / G5 changelog entries) — *casual-surface V-form leak is real but the proven `ru` anchor is itself V-form on that same key, accept-condition (b) unmet → RU-PARITY-DEFER* — is **closed by operator decision**. The evidenced genuinely-casual ru V-leak set was re-audited and corrected to T-form in `ru.lproj`: `yearResultsPushTitleTemp`, `didgestSettingsTitle`, `pressOnPlusToAdd`, `drinkParamsNotionMessage`, `youShuldAddToFriend`, `notifSoundText`, `drinksReorderText` (canonical owner — `TRANSLATION_STYLE.md § Brand voice § Pronouns`). Accept-condition (b) is now MET for these keys (ru is T-form) → normal **accept** path, not defer. RU-PARITY-DEFER remains in the validator only as a safety net for any *unforeseen new* ru-V-on-casual key — NOT a known-open cluster; G6 (hi / ar) must NOT expect a defer cluster on these keys. Face A (paywall-CTA / hero) was codified intentional single-register formal Pre-G6 — **deprecated 2026-05-30** (split retired; paywall is now T-form too, the `reachTheGoal` casual-T is the norm); legacy «вы» there is grandfathered — see skip rule #1 + the 2026-05-30 entry.

   **T-V / honorific map (the friendly T-form per language is the universal target; the V / formal column is deprecated legacy-reference for detecting leaks, NOT a valid register for new strings):**
...
1. **Formal V-form / honorific in formal surfaces — DEPRECATED (2026-05-30, split retired).** This carve-out is **no longer in force**: per `TRANSLATION_STYLE.md § Brand voice § Pronouns` friendly T-form is the universal register, and the former-formal surfaces — App Store metadata (`appstore.app.*`), paywall marketing hero / CTA (`free.title*`, screenshot taglines, `reachTheGoal`), onboarding medical-authority educational, legal / privacy notices, permission prompts (`NS*UsageDescription`), error-with-recovery (`noConnection`, `noFreeStorage`, `noRamError`, `favoriteDrinkCreateFailed`, watch / Siri / Premium fallback errors), Siri AppIntent description / `siriPlaceHolder` — are **no longer V-form by design**. Operational consequence in **two** parts: (a) do **not** *defend* a V-form target on these surfaces as "intentional formal register" — that justification is gone; (b) but because the operator declined a re-translation sweep, their existing legacy «вы» is **grandfathered** — do **not** enumerate it as a project-wide must-fix either (no mass sweep; vendor console owns translations — `CM-LOCALE-MASS-FANOUT`). Net on former-formal surfaces: neither defend nor sweep legacy «вы»; flag only a clearly new / source-changed string, severity `warn`. Applies to ALL T-V / honorific languages (replace V-form with the target-language equivalent: Sie, vous, usted, Lei, o senhor, Pan/Pani, siz, u, 您, आप, Anda, anda, formal kinship, 敬語, 합쇼체, fuṣḥā formal). **Legacy formal-form в casual ключах** (e.g., `pushRetention_*`, `tempAddOneTap`, `forgotWriteText`, `manyDrinksText`, `noAdText`, `tempFirstSteps`, `widgetText`, `NewYearMotivationalText`) — still **flag** as `brand-voice` per audit rule #8 (casual sweep scope, unchanged).
...
Flag ru imperative ending in `-ите` / `-айте` / `-йте` / `-ьте` (`добавьте`, `выпейте`, `пейте`, `начните`, `следуйте`, `оставайтесь`, `не забывайте`, `позаботьтесь`, `используйте`, `откройте`, `попробуйте`, `создайте`, `выберите`, `сделайте`, `нажмите`, `проверьте`, `отслеживайте`, `учитывайте`, `записывайте`, `рассчитайте`, `поставьте`), OR pronoun `вы` / `вас` / `вам` / `ваш` / `ваша` / `ваше` / `ваши` in app voice. Per `TRANSLATION_STYLE.md § Brand voice § Pronouns` — friendly T-form («ты») is the **universal** brand-voice register (split deprecated 2026-05-30); V-form is off-brand on every surface. Operationally: flag casual-surface V-form (confirmed-defect sweep scope); former-formal-surface legacy «вы» is grandfathered (advisory `warn` only, no mass sweep) — see rule #8 + skip rule #1 (deprecated).
...
- **rule #8 T-V/honorific map (all 21) + binding OPERATOR POLICY** — Phase 1 inlined the per-language casual/formal map (Scandinavian = "skip #8 entirely, no morphological T-V"); G1 made casual-surface V-form a **confirmed project-wide defect** (enumerate per-row, no Summary-consolidation / no deferral, validator must accept). Carried ru-parity tension CLOSED Pre-G6. **2026-05-30: generalized casual → ALL surfaces** (split deprecated, friendly T-form universal); the casual sweep scope is unchanged, former-formal surfaces are grandfather (advisory `warn`, no sweep).
...
- **2026-05-30 — `ты`/`вы` split DEPRECATED; friendly T-form universal (operator decision; doc-only, grandfather, NO sweep):**
