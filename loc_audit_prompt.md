...
8. **Formal V-form imperative in casual surface** — flag when target uses V-form / honorific / formal register in a **casual** surface (notification, motivational, tip, award, empty state, beverage / achievement name, widget hint, in-app feature card / upsell). Per `TRANSLATION_STYLE.md § Brand voice § Pronouns` — friendly T-form is brand-voice default for casual surfaces; V-form is exception only for formal surfaces explicitly listed in § What NOT to flag rule 1.
   ...
   **Key-level `Register:` field — authoritative override when present.** If the entry's comment block carries an explicit `Register:` line (`Register: casual T-form` / `Register: formal V-form` / `Register: neutral`; canonical owner — `TRANSLATION_STYLE.md § Translator context § Опциональные поля`), treat it as the authoritative register decision for that key:
   ...
   - **Absence of `Register:` does NOT mean casual.** Fall back to surface-based default (skip rule #1 formal-surface list vs casual default). After the 2026-05-28 casual-Type backfill the field sits at ~18% corpus-wide and **100%** on the casual-Type leaf set ({tip, tip headline, motivational text, notification body / title, achievement description} — the historically largest V-form-leak class); on other surfaces absence is still the norm, not a casual signal.
