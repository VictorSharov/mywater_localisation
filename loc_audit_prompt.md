...
7. **Broken placeholders** — count of `%@` / `%d` / `%.0f` / `%lld` in en vs what `Placeholders` field in comment describes. NOTE: `%%` is a required escape for a literal `%` sign — R.swift wraps every accessor in `String(format:)`, so a lone `%` is undefined behavior. Flag a lone `%` in en source as a bug, and treat `%%` in target as correct (not a placeholder mismatch).
...
4. **Placeholder count** must match en exactly (same `%@` / `%d` / `%.0f` positions; reordering via `$1` / `$2` indexing is fine when target grammar requires it — see ru placeholder swap example above). `%%` in target is the **correct** escape for a literal `%` — do NOT flag as a placeholder count mismatch even when en has a lone `%`. The en source with lone `%` is the bug; target preserving `%%` is correct.
...
8. **Formal V-form imperative in casual surface** — flag when target uses V-form / honorific / formal register in a **casual** surface (notification, motivational, tip, award, empty state, beverage / achievement name, widget hint, in-app feature card / upsell). Per `docs/LOCALIZATION.md § Brand voice § Pronouns` — friendly T-form is brand-voice default for casual surfaces; V-form is exception only for formal surfaces explicitly listed in § What NOT to flag rule 1. Failure mode catalog: `[CM-LOCALE-V-FORM-LEAK]`.
...
Per `docs/LOCALIZATION.md § Comment context → Constraints`:

**Hard binding constraints (flag when target violates):**
- Exact `%@` / `%d` placeholder count & order.
...
Reject these terms in ru translations (canonical owner — `docs/LOCALIZATION.md § Translation discipline § Принципы § 3`):
...
Flag ru imperative ending in `-ите` / `-айте` / `-йте` / `-ьте` (`добавьте`, `выпейте`, `пейте`, `начните`, `следуйте`, `оставайтесь`, `не забывайте`, `позаботьтесь`, `используйте`, `откройте`, `попробуйте`, `создайте`, `выберите`, `сделайте`, `нажмите`, `проверьте`, `отслеживайте`, `учитывайте`, `записывайте`, `рассчитайте`, `поставьте`), OR pronoun `вы` / `вас` / `вам` / `ваш` / `ваша` / `ваше` / `ваши` used in a **casual** surface. Per `docs/LOCALIZATION.md § Brand voice § Pronouns` — friendly T-form («ты») is brand-voice default for casual surfaces; V-form is exception only for formal surfaces (skip rule #1).
...
- `mywater_ios docs/LOCALIZATION.md § Translation discipline` — canonical linguistic rules (cross-repo); `§ Shared localisation corpus & tooling` — pipeline overview.
